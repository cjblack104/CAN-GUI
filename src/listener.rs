...
fn main() {
	...
	let mut hndl : i16;
	/*
	Ignore the UI for now - implement listener and thread pool only
...
fn handle_connection(mut stream: TcpStream) {
	...
	// Define protocol to use with TCP
	// Each unique request needs its own ID to be checked in the If statement (possibly make this a switch statement)
